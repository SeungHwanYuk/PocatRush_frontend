      ...
      <CardContainer>
        {ranking && ranking.map((character) => (
          <Card key={character.rank} $top3={character.rank <= 3}>
            ...
            <CardContent>
              <CardTitle $top3={character.rank <= 3}>{character.charNickName}</CardTitle>
              <CardText $top3={character.rank <= 3} style={{ color: getMedalColor(character.rank), fontWeight: 'bold' }}>
                {character.rank === 1 ? '1위' : character.rank === 2 ? '2위' : character.rank === 3 ? '3위' : `${character.rank}위`}
              </CardText>
              <CardText $top3={character.rank <= 3}>EXP: {character.exp}</CardText>
            </CardContent>
          </Card>
        ))}
      </CardContainer>
